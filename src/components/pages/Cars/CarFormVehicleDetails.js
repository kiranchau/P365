...
export default function CarFormVehicleDetails(props) {
  ...
  const [vechicleInfoFromRto, setvechicleInfoFromRto] = React.useState();
  ...
  const [InsuranceFor, setInsuranceFor] = useState([
    ...
    { value: "new", label: "Insure New Car" },
  ...
  useEffect(() => {
    // Set a timeout to delay the execution of the following code by 2000 milliseconds (2 seconds)
    const getData = setTimeout(() => {
      // Check if the vehicle registration number is longer than 8 characters,
      // the RTO data has a user ID, and the text is not already visible
      if (props.formik.values.registeredRto?.length > 90 && formContext.rtoData?.userId && !isTextVisible) {
        ...
        api.rtoGetData(formContext.rtoData?.userId, formContext.rtoData?.id, props.formik.values.registeredRto).then(
          ...
          console.log("err", err)
          if (err.response.data.message === "Vehicle Number is not valid") {
      ...
      // If the vehicle registration number is longer than 9 characters and there's no user ID in RTO data
      else if (
        props.formik.values.registeredRto?.length > 9 &&
        !formContext.rtoData?.userId
      ) {
        formContext.setLoginData();
        // Set the formik field value for registered RTO
    ...
    // Cleanup function to clear the timeout when the component is unmounted or when the dependency changes
    return () => clearTimeout(getData);
  }, [props.formik.values.registeredRto]);

  useEffect(() => {
    formContext?.setselectedRto("")
    formContext.makeModelApi("Car")
  ...
  useEffect(() => {
    if (vechicleInfoFromRto?.model) {
      ...
      props.formik.setFieldValue("make", vechicleInfoFromRto.vehicleManufacturerName)
      props.formik.setFieldValue("model", vechicleInfoFromRto.model)
      props.formik.setFieldValue("fuel", vechicleInfoFromRto?.type?.toLowerCase())
  ...
  const handlePolicyExpDate = (vehicleInsuranceUpto) => {
    ...
    if (vahanDate < valuesForExpDate?.priorDate) {
      ...
      props.formik.setFieldValue("previousPolicy", "notexpired", valuesForExpDate?.todayConvertedValue)
    }
  }
  ...
  // Handles the click event on a span element.
  const handleSpanClick = () => {
    ...
    // If text was visible before the click, focus on the registeredRtoInputRef
    if (isTextVisible) {
      registeredRtoInputRef.current.focus();
    } else {
      formContext.setModalShow(true);//If text was not visible before the click, show a modal by setting modal show state to true
    }
  };
    ...
    <Form as="div" className="form-bg">
      <>
        ...
        <CarPolicy
          ...
          message="As per RTO this vehicle belongs to Two Wheeler category."
          type={1}
          formik={props.formik} />
      </>
      <Row>
        ...
        <Col md={12} sm={6} className=" mb-4">
          <InputFieldDropdown
            ...
            formik={props.formik}
          />
          <ErrorMessage formik={props.formik} fieldValue="fuel" />
        </Col>
